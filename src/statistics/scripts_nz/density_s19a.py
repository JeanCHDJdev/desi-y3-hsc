...
cut_names = [
    ...
    "i_pixelflags_edge == 0",
    "i_hsmshaperegauss_flag == 0",
    "i_hsmshaperegauss_sigma != NaN",
    "i_extendedness_value != 0",
    "i_cm_flux/err >= 10 (S/N)",
    "i_hsmshaperegauss_resolution >= 0.3",
    "R2 >= 0.3 (resolution cut)",
    "i_hsmshaperegauss_e1**2 + i_hsmshaperegauss_e2**2 < 4",
    "0 <= i_hsmshaperegauss_sigma <= 0.4",
    "i_cm_mag - a_i <= 24.5",
    "i_cm_mag - a_i <= 25",
    "i_cm_mag - a_i <= 25.5",
...
for idx, name in enumerate(cut_names):
    ...
    elif idx == 12: cut = hsc_base["i_hsmshaperegauss_flag"] == 0
    elif idx == 13: cut = hsc_base["i_hsmshaperegauss_sigma"] >= 0
    elif idx == 14: cut = hsc_base["i_extendedness_value"] != 0
    elif idx == 15: cut = hsc_base['i_cm_flux']/hsc_base['i_cm_fluxerr'] >= 10
    elif idx == 16: cut = hsc_base["resolution"] >= 0.3
    elif idx == 17: cut = 0.7*(hsc_base['i_sdssshape_shape11'] + hsc_base['i_sdssshape_shape22']) >= hsc_base['i_sdssshape_psf_shape11'] + hsc_base['i_sdssshape_psf_shape22']
    elif idx == 18: cut = hsc_base["e_1"]**2 + hsc_base["e_2"]**2 < 4
    elif idx == 19: cut = ((hsc_base["i_hsmshaperegauss_sigma"] >= 0) &
                           (hsc_base["i_hsmshaperegauss_sigma"] <= 0.4))
    elif idx == 20: cut = hsc_base['i_cm_mag'] - hsc_base['a_i'] <= 24.5
    elif idx == 21: cut = hsc_base['i_cm_mag'] - hsc_base['a_i'] <= 25
    elif idx == 22: cut = hsc_base['i_cm_mag'] - hsc_base['a_i'] <= 25.5
    elif idx == 23: cut = hsc_base["i_apertureflux_10_mag"] <= 25.5
    elif idx == 24: cut = hsc_base["i_blendedness_abs"] < 0.416869
    elif idx == 25: cut = ((hsc_base['g_inputcount_value'] >= 1) &
                            (hsc_base['r_inputcount_value'] >= 1) &
                            (hsc_base['i_inputcount_value'] >= 1) &
                            (hsc_base['z_inputcount_value'] >= 1) &
                            (hsc_base['y_inputcount_value'] >= 1))
    elif idx == 26: cut = ((hsc_base['g_inputcount_value'] >= 4) &
                            (hsc_base['r_inputcount_value'] >= 4) &
                            (hsc_base['i_inputcount_value'] >= 5) &
                            (hsc_base['z_inputcount_value'] >= 5) &
                            (hsc_base['y_inputcount_value'] >= 5))
    elif idx == 27: cut = ~((hsc_base['ra'] >= 132.5) & (hsc_base['ra'] <= 140.0) & 
                            (hsc_base['dec'] >= 1.6)  & (hsc_base['dec'] <= 5.0))
    elif idx == 28: cut = hsc_base["z_bin"] >= 1
    elif idx == 29: cut = hsc_base["b_mode_mask"] == 1
    elif idx == 30: cut = ((hsc_base['i_detect_isprimary'] == 1) & 
    ...
    print(f"  3.5 <= z        : {density_last:.{accuracy}g} arcmin^-2 (N={n_last})")
    print("-" * 60)