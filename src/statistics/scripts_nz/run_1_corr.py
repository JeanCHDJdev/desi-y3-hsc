...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="params.yml",
        help="Path to configuration YAML file. Default is params.yml",
    )
    parser.add_argument(
        "-a",
        "--areas",
...
def main():
    ...
    for m in areas:
        ...
        for t1, t2 in zip(tgt1, tgt2):
            ...
            if t2 == "HSC":
                borns = [hsc_keys[0], hsc_keys[-1] + 1]
            else:
                borns = [1, len(bin2)]
            
            for b1 in range(1, len(bin1)):
                for b2 in range(*borns):
                    ...
                    if calib_photoz_bias:
                        if t2 == "HSC":
                            ...
                            if (
                                ...
                                continue
                        if desi_version == "dr1" and (bin2[b2 - 1] + bin2[b2]) / 2 > 0.9:  # > if DR1, < if DR2
                            logger.info(
                                f"Skipping cross-correlation for {t1}x{t2}, "
                                f"bin 1 {b1} : {bin1[b1-1]:.2f}-{bin1[b1]:.2f}, bin 2 {b2} : {bin2[b2-1]:.2f}-{bin2[b2]:.2f} "
                                f"(z > 0.9)"
                            ...
                            continue
                        elif desi_version == "dr2" and (bin2[b2 - 1] + bin2[b2]) / 2 < 0.9:  # > if DR1, < if DR2
                            logger.info(
                                f"Skipping cross-correlation for {t1}x{t2}, "
                                f"bin 1 {b1} : {bin1[b1-1]:.2f}-{bin1[b1]:.2f}, bin 2 {b2} : {bin2[b2-1]:.2f}-{bin2[b2]:.2f} "
                                f"(z > 0.9)"
                    ...
                    tb1b2 = time.time()
                    cc.run(b1, b2 - borns[0] + 1, m, data_release=desi_version, hsc_cat=hsc_catalog)
                    txt = (
                        f"Finished {t1}x{t2}, {b1} : "
                        f"{bin1[b1-1]:.2f}-{bin1[b1]:.2f}, "
                        f"{b2} : {bin2[b2-borns[0]]:.2f}-{bin2[b2-borns[0]+1]:.2f} in {time.time()-tb1b2:.2f}s"
                    )
                    logger.info(txt)

            cc.save_zeff(t1, t2, m, data_release=desi_version, hsc_cat=hsc_catalog)
