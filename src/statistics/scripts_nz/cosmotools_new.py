...
config = CorrelationConfig.get_current()
username = config.username
hsc_catalog = config.hsc_catalog
...
def alpha_mag(hsc_cat, mag_bins, imag_cut, zbin_index, z_range):
    if zbin_index in [1, 2, 3, 4]:
        hsc_zbin = hsc_cat[["i_cm_mag", "a_i"]][hsc_cat["z_bin"] == zbin_index]
    else:
        hsc_zbin = hsc_cat[["i_cm_mag", "a_i"]][(hsc_cat["dnnz_photoz_best"] >= z_range[0]) & (hsc_cat["dnnz_photoz_best"] < z_range[1])]
    # if len(hsc_zbin) == 0:
    #     zbin = hsc_cat["z_bin"]
    #     photoz = hsc_cat["dnnz_photoz_best"]
    #     zbin[(photoz >= config.bins_hsc[zbin_index-config.hsc_keys[0]]) & (photoz < config.bins_hsc[zbin_index-config.hsc_keys[0]+1])] = zbin_index
    #     hsc_zbin = hsc_cat[["i_cm_mag", "a_i"]][hsc_cat["z_bin"] == zbin_index]
    imag = hsc_zbin["i_cm_mag"] - hsc_zbin["a_i"]
    mag_counts = cumulative_mag_counts(mag_bins, imag)
    valid_mag_counts = mag_counts[mag_counts > 0]
    valid_mag_bins = mag_bins[mag_counts > 0]
    # fit a 4th order polynomial to the cumulative counts
    poly_mag = np.poly1d(np.polyfit(valid_mag_bins, valid_mag_counts, 4))
    poly_mag_der = np.polyder(poly_mag, 1)
    # composite derivative: dlogP/dm = (1 / (P * ln(10))) * dP/dm
    s_lim = poly_mag_der(imag_cut) / (poly_mag(imag_cut) * np.log(10))
    alpha = 2.5*s_lim - 1
    return alpha

def parametrize_bias(tracer, tomo_bin, wdm, scale_cut, imag_cut):
    """
    ...
    if hsc_catalog == 'hscy3_cat_withflags' or hsc_catalog == 'hscy3_cat' or hsc_catalog == 'hscy3_cat_no_bmode_cut' or hsc_catalog == "hscy3_z-cut": # for S19A use exact alpha coef for each sub-bin
        print(f"Using data values for alpha_p coefficients for {hsc_catalog}.")
        # alpha_p individual values computation
        hsc_cat = None
        
        if imag_cut is None:
            hsc_path = f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/hsc/cat/{hsc_catalog}.fits"
            hsc_cat = fio.FITS(hsc_path)[1].read()
            i_cm_mag = hsc_cat["i_cm_mag"]
            a_i = hsc_cat["a_i"]
            imag_cut = float(f"{np.max(i_cm_mag - a_i):.3g}")
        else:
            imag_cut = float(imag_cut)
        
        with open("alpha_dic.yml", "r") as f:
            alpha_dic = yaml.safe_load(f) or {}
        
        z_ranges = []
        z_range_keys = []
        alpha_bin = [0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.75, 2.0, 2.5, 3.0, 3.5]
        
        if tomo_bin in [1, 2, 3, 4]:
            z_range = tuple(config.bins_hsc_raw[tomo_bin])
            z_ranges.append(z_range)
            z_range_keys.append(f"{z_range[0]}-{z_range[1]}")
        else:
            for i in range(len(alpha_bin)-1):
                z_min = alpha_bin[i]
                z_max = alpha_bin[i+1]
                z_ranges.append((z_min, z_max))
                z_range_keys.append(f"{z_min}-{z_max}")
        
        tomo_bin_key = tomo_bin if tomo_bin in [1, 2, 3, 4] else 0
        
        ap_per_range = {}
        for z_range, z_range_key in zip(z_ranges, z_range_keys):
            need_compute = (
                tomo_bin_key not in alpha_dic or 
                z_range_key not in alpha_dic[tomo_bin_key] or
                hsc_catalog not in alpha_dic[tomo_bin_key][z_range_key] or 
                imag_cut not in alpha_dic[tomo_bin_key][z_range_key][hsc_catalog] or
                alpha_dic[tomo_bin_key] is None or
                alpha_dic[tomo_bin_key][z_range_key] is None or
                alpha_dic[tomo_bin_key][z_range_key][hsc_catalog] is None or
                alpha_dic[tomo_bin_key][z_range_key][hsc_catalog][imag_cut] is None
                )

            if need_compute:
                if tomo_bin_key == 0:
                    print(f"Computation required for z-range {z_range_key}, catalog {hsc_catalog} at imag cut = {imag_cut}...")
                else:
                    print(f"Computation required for bin {tomo_bin_key}, catalog {hsc_catalog} at imag cut = {imag_cut}...")
                if hsc_cat is None:
                    hsc_path = f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/hsc/cat/{hsc_catalog}.fits"
                    hsc_cat = fio.FITS(hsc_path)[1].read()

                mag_bins = np.linspace(imag_cut - 0.5, imag_cut + 0.2, 101)
                valeur_alpha = float(f"{alpha_mag(hsc_cat=hsc_cat, mag_bins=mag_bins, imag_cut=imag_cut, zbin_index=tomo_bin, z_range=z_range):.3g}")

                alpha_dic.setdefault(tomo_bin_key, {}).setdefault(z_range_key, {}).setdefault(hsc_catalog, {})[imag_cut] = valeur_alpha

                # save new file
                with open("alpha_dic.yml", "w") as f:
                    yaml.dump(alpha_dic, f, default_flow_style=False)
            
            ap_per_range[z_range_key] = alpha_dic[tomo_bin_key][z_range_key][hsc_catalog][imag_cut]
        
        def localize_z_range_key(z, alpha_bin):
            idx = bisect.bisect_right(alpha_bin, z)
            idx = max(1, min(idx, len(alpha_bin) - 1))
            return f"{alpha_bin[idx - 1]}-{alpha_bin[idx]}"
        
        def get_ap_value(z, tomo_bin_key):
            if tomo_bin_key == 0:
                z_range_key = localize_z_range_key(z, alpha_bin)
                return ap_per_range[z_range_key]
            else:
                z_range = tuple(config.bins_hsc_raw[tomo_bin_key])
                z_range_key = f"{z_range[0]}-{z_range[1]}"
                return ap_per_range[z_range_key]
        
        alpha_model_p = lambda z: get_ap_value(z, tomo_bin_key)
    
    else: # for pdr3 catalog use a linear fit
        print(f"Using linear fit formula for alpha_p coefficients for {hsc_catalog}.")
        # alpha_p fit values
        hsc_cat = None
        ...
        with open("alpha_fit.yml", "r") as f:
            alpha_fit = yaml.safe_load(f) or {}
        
        need_compute = (hsc_catalog not in alpha_fit or
                        imag_cut not in alpha_fit[hsc_catalog])
        
        if need_compute:
            print(f"Computation required for catalog {hsc_catalog} at imag cut = {imag_cut}...")
            
            if hsc_cat is None:
                hsc_cat = fio.FITS(f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/hsc/cat/{hsc_catalog}.fits")[1].read()
            
            z_interval = np.arange(0, 3.55, 0.1)
            z_alpha = []
            alpha = []
            mag_bins = np.linspace(24, 24.7, 101)
            ...
            for i in range(len(z_interval)-1):
                zmin = z_interval[i]
                zmax = z_interval[i+1]
                z_alpha.append((zmin + zmax)/2)
                print(f"Processing z-range {zmin}-{zmax}")
                zbin_bis = hsc_cat[(hsc_cat["dnnz_photoz_best"] >= zmin) & (hsc_cat["dnnz_photoz_best"] < zmax)]
                imag = zbin_bis["i_cm_mag"] - zbin_bis["a_i"]
                print(f"Number of objects in z-range {zmin}-{zmax}: {len(imag)}")
                mag_counts = cumulative_mag_counts(mag_bins, imag)
                valid_mag_counts = mag_counts[mag_counts > 0]
                valid_mag_bins = mag_bins[mag_counts > 0]
                # fit a 4th order polynomial to the cumulative counts
                ...
                poly_mag_der = np.polyder(poly_mag, 1)
                # composite derivative: dlogP/dm = (1 / (P * ln(10))) * dP/dm
                s_lim = poly_mag_der(24.5) / (poly_mag(24.5) * np.log(10))
                alpha.append(2.5*s_lim - 1)
            
            from scipy.stats import linregress
            resultats = linregress(z_alpha, alpha)
            a_opt, b_opt = float(f"{resultats.slope:.3g}"), float(f"{resultats.intercept:.3g}")
            alpha_fit.setdefault(hsc_catalog, {})[imag_cut] = {'a_opt' : a_opt, 'b_opt' : b_opt}

            with open("alpha_fit.yml", "w") as f:
                yaml.dump(alpha_fit, f, default_flow_style=False)
            
        a_opt, b_opt = alpha_fit[hsc_catalog][imag_cut]['a_opt'], alpha_fit[hsc_catalog][imag_cut]['b_opt']
        alpha_model_p = lambda z: a_opt*z + b_opt
        
    #alpha_model_p = lambda z: alpha_mag(hsc_cat=hsc_cat, mag_bins=mag_bins, imag_cut=imag_cut, zbin_index=tomo_bin)
    #match tomo_bin:
        # case 1:
...
def solve_magnification(
    ...
    alpha_p, alpha_s, bias_p, bias_s = parametrize_bias(
        tracer=tracer, tomo_bin=tomo_bin, wdm=w_dm_interp, scale_cut=scale_cut, imag_cut=imag_cut
    )
