...
def wss(
    ...
    if frNGC is not None:
        for moc_id in mocngc:
            filengc = frNGC.get_file(bin_index1, bin_index2, tracer1, tracer2, moc_id, data_release, hsc_cat)
            try:
    ...
    if frSGC is not None:
        for moc_id in mocsgc:
            filesgc = frSGC.get_file(bin_index1, bin_index2, tracer1, tracer2, moc_id, data_release, hsc_cat)
            try:
...
def wpp(
    ...
    files = fr.get_file(bin_index, bin_index, "HSC", "HSC", moc=which_patches, data_release=data_release, hsc_cat=hsc_cat)
...
def wsp(
    ...
    files = fr.get_file(fine_bin, tomo_bin, tracer, "HSC", moc=which_patches, data_release=data_release, hsc_cat=hsc_cat)
...
def compute_npz(
    ...
    if do_spec_correction:
        qso_spectro_bias = config.qso_spectro_bias
        
        if tracer == "QSO" and qso_spectro_bias == "Laurent":
            bs_meas = 0.278 * ((1 + zloc)**2 - 6.565) + 2.393
            bs_err = np.sqrt((((1 + zloc)**2 - 6.565)*0.018)**2 + 0.042**2)
            if do_phot_correction:
                factor = np.sqrt(deltaz * precomp_wdm)
            else:
                factor = precomp_wdm
            result = wsp_meas / (bs_meas * factor)
            combined_err = np.abs(result) * np.sqrt((wsp_err / wsp_meas)**2 + (bs_err/bs_meas)**2)
            
        elif tracer == "QSO" and qso_spectro_bias == "Charles":
            bs_meas = 0.230 * ((1 + zloc)**2 - 6.565) + 2.394
            bs_err = np.sqrt((((1 + zloc)**2 - 6.565)*0.007)**2 + 0.035**2)
            if do_phot_correction:
                factor = np.sqrt(deltaz * precomp_wdm)
            else:
                factor = precomp_wdm
        ...
        elif tracer != "QSO" or qso_spectro_bias == "wss":
            if do_phot_correction:
            ...
            raise ValueError(
            "qso_spectro_bias should be one of 'wss', 'Laurent', 'Charles'."
        )
        n_z_bs = result
    ...
    if do_phot_correction:
        ...
        wpp_err = 2 * np.abs(wpp_meas) * np.sqrt((delta_alpha/alpha)**2 + (np.log(1 + zloc) * delta_gamma)**2)
        
        result_prealpha = n_z_bs / ((1 + zloc) ** gamma)
        result = n_z_bs / (alpha * ((1 + zloc) ** gamma))
        ...
        combined_err = np.sqrt(
            ...
            + (result_prealpha * delta_alpha / alpha**2) ** 2
        )
...
def compute_npz_merged(
    ...
    fine_redshift = _get_fine_redshift_bins(cf.CorrFileReader(path_dictionary["DESIxHSC"]), tracer=tracer)
    ...
    file_ss = frss.get_file(fine_bin, fine_bin, "Merged", "Merged", "Merged", data_release, hsc_cat)
    frsp = cf.CorrFileReader(path_dictionary["MergedxHSC"])
    file_sp = frsp.get_file(fine_bin, tomo_bin, "Merged", "HSC", "Merged", data_release, hsc_cat)
...
def _get_fine_redshift_bins(fr: cf.CorrFileReader, tracer="Merged"):
    ...
    dz = dzall[0]
    del dzall
    # include the upper edge maxt so the last fine bin reaches the provided max
    # (use maxt + dz/2 to mimic the inclusive behavior used elsewhere)
    fine_redshift = np.arange(mint, maxt + dz / 2.0, dz)
...
def full_rcc(
    path_dictionary, tracer1, tracer2, data_release, hsc_cat, rebin=1, verbose=False, scale_cuts=None
):
    """
...
def merge_estimators(
                ...
                if np.isclose(zt, zr, atol=dz / 5):
                    ...
                    if fr_auto_NGC is not None:
                        ...
                        paths_autos_z.extend(
                            fr_auto_NGC.get_file(zindt, zindt, t, t, moc=[1], data_release=data_release, hsc_cat=hsc_cat)
                        )
                    if fr_auto_SGC is not None:
                        # moc 3 + skip moc is by default the SGC config... not very clear I know
                        paths_autos_z.extend(
                            fr_auto_SGC.get_file(zindt, zindt, t, t, moc=[3], data_release=data_release, hsc_cat=hsc_cat)
                        )
                    assert len(paths_autos_z) > 0, (
                        "No valid autocorrelations. got "
                        f"{fr_auto_NGC.get_file(zindt, zindt, t, t, None, data_release=data_release, hsc_cat=hsc_cat)}"
                    )
                    ...
                    for index_tomo, hsc_tomo in enumerate(which_tomo, start=1):
                        ...
                        paths_tomo_cross = fr_cross.get_file(
                            zindt, hsc_tomo, t, "HSC", moc=which_patches, data_release=data_release, hsc_cat=hsc_cat
                        )
    ...
    for i in range(1, len(redshift_bin_centers) + 1):
        for j, (est, tomo_bin) in enumerate(
            ...
            file_path = cross_dir / f"MergedxHSC_b1x{i}_b2x{tomo_bin}_{data_release}_{hsc_cat}.npy"
        ...
        file_path = autos_dir / f"MergedxMerged_b1x{i}_b2x{i}_{data_release}.npy"
...
def merge_results(
    ...
    if not any(valid_z_range):                             # copilot 21/05
        print(
            "Warning: No valid z range found after merging. "
            "This means all input error arrays were zero or invalid for the given correction and tomographic bin."
        )
        return np.array(zvals), np.array([]), np.array([])
