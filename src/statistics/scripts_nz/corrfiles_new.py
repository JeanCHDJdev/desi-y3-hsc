...
corr_title_nz = config.corr_title_nz
if len(corr_title_nz) > 0:
    corr_title_nz += '_'
...
class CorrFileReader:
    ...
    def get_file(self, b1, b2, tgt1, tgt2, moc, data_release, hsc_cat):
        """
        ...
        if moc == "Merged":
            if tgt2 == "HSC":
                return f"{DIR}/{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_{data_release}_{hsc_cat}.npy"
            else:
                return f"{DIR}/{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_{data_release}.npy"
        if moc is None:
            moc = [1, 2, 3, 4]
        if isinstance(moc, list):
            assert all(
                m in [1, 2, 3, 4] for m in moc
            ), f"MOC values should be in [1, 2, 3, 4], not {moc}"
            path_list = []
            if tgt2 == "HSC":
                list_files = sorted(
                    list(
                        Path(f"{DIR}", f"{corr_title_nz}{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_moc{m}_{data_release}_{hsc_cat}.npy")
                        for m in moc
                    )
                )
            else:
                list_files = sorted(
                    list(
                        Path(f"{DIR}", f"{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_moc{m}_{data_release}.npy")
                        for m in moc
                    )
                )
            assert len(list_files) == len(
                moc
            ), f"Expected {len(moc)} files, found {len(list_files)} for {tgt1}x{tgt2} b1x{b1} b2x{b2}"
        ...
        if isinstance(moc, int):
            assert moc in [
                ...
                3,
                4,
            ], f"MOC should be an integer in [1, 2, 3, 4], not {moc}"
            if tgt2 == "HSC":
                return f"{DIR}/{corr_title_nz}{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_moc{moc}_{data_release}_{hsc_cat}.npy"
            else:
                return f"{DIR}/{tgt1}x{tgt2}_b1x{b1}_b2x{b2}_moc{moc}_{data_release}.npy"

    def get_auto_file(self, b, tgt, moc, data_release, hsc_cat):
        """
        Get the file name for given redshift bins and MOC.
        """
        return self.get_file(b, b, tgt, tgt, moc, data_release, hsc_cat)
    ...
    try:
        if sims:
            ...
            if version not in ["DR1", "DR2"]:
                raise ValueError(
                    f"Unknown version {version}. Available versions are 'DR1' and 'DR2'."
                )

            if version == "DR2":
                root = Path(
                    "/global/cfs/projectdirs/desi/survey/catalogs/Y3/LSS/loa-v1/LSScats/v2/"
                )
            elif version == "DR1":
                root = Path(
                    "/global/cfs/projectdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v1.5/"
                )
...
def fetch_hsc_files(randoms=False, include_dud=False, sims=False, sims_version=0):
    try:
        if sims:
            sims_root = (
                f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/sims/"
            )
            if randoms:
                return Path(
                    sims_root, "randoms", f"HSC_randoms_zcorr_v{sims_version}.fits"
                )
            return Path(
                sims_root, f"v{sims_version}", f"hscy3_sim_v{sims_version}.fits"
            )
        elif randoms:
            # this path root currently does not contain D/UD randoms as they
            root = Path(
                f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/hsc/randoms"
            )
            return list(
            ...
            return Path(
                f"/global/cfs/projectdirs/desi/users/{username}/desi-y3-hsc/data/hsc/cat/{hsc_catalog}.fits"
            )
    except PermissionError:
