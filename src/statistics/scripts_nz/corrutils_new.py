...
desi_version = config.desi_data_release
hsc_catalog = config.hsc_catalog
overwrite_files = config.overwrite_files
corr_title_corr = config.corr_title_corr
calibration_cut = config.calibration_cut
if len(corr_title_corr) > 0:
    corr_title_corr += "_"
...
class CorrelationMeta(ABC):
    ...
    data_release = f"DR{desi_version[-1]}"
    ...
    bins_bgs = config.bins_bgs  # 0 <= z < 0.6
    bins_lrg = config.bins_lrg  # 0.3 <= z < 1.2
    bins_elg = config.bins_elg  # 0.7 <= z < 1.6
    bins_qso = config.bins_qso  # 0.7 <= z < 2.8
    bins_hsc = config.bins_hsc
    ...
    bins_mode = {"rp": bins_rp, "rppi_s": bins_rppi_s, "rppi_mu": bins_rppi_mu}
    ...
    def save_bins(root):
        ...
        if outfile.exists():
            print(
                f"Bins file {outfile} already exists, saving new iteration under a different name"
            )
            outfile = Path(bin_dir, f'bins_all_{time.strftime(("%Y%m%d_%H%M%S"))}.npz')

        np.savez(outfile, **{k: v for k, v in CorrelationMeta.bins_all.items()})
    ...
    def __init__(
        ...
        assert (
            tgt1 is not None and tgt2 is not None
        ...
        self.logger.info(
            f"Using data version {self.data_release} for targets {self.tgt1} and {self.tgt2}"
        )
        ...
        if self.corr_type == "rppi":
            ...
            self.edges = self.bins_mode[
                "rp" if self.corr_type == "theta" else self.corr_type
            ]
        ...
        self.make_cats(
            bin_redshift1=self.bin_redshift1, bin_redshift2=self.bin_redshift2
        )
    ...
    def set_desi_tracer(self, tgt, bin_redshift):
        ...
        self.logger.info(
            f"Read DESI data in {time.time()-tid:.2f} seconds ({len(cat)} rows)"
        )
        ...
        zmask_dat = np.digitize(cat[self.z_desi_col], bin_redshift, right=True)
        zmask_ran = np.digitize(ran[self.z_desi_randoms_col], bin_redshift, right=True)
    ...
    def set_hsc_tracer(self, bin_redshift):
        ...
        if self.sims:
            zmask_ran = np.digitize(
                ran[self.z_hsc_randoms_col], bin_redshift, right=True
            )
        else:
        ...
        if self.use_zbin:
            ...
            zmask_data = np.digitize(zvalues, bin_redshift, right=True)
            ...
            inside_tomo_range = (zvalues > ztomographic[0]) & (
                zvalues <= ztomographic[1]
            )
            bad_quality = zmask_bins == 0

            # implementing the fiducial cut with the difference in
            diffmizuki = (
                cat["mizuki_photoz_err95_max"][:] - cat["mizuki_photoz_err95_min"][:]
            )
            diffdnnz = cat["dnnz_photoz_err95_max"][:] - cat["dnnz_photoz_err95_min"][:]
            quality_mask = (diffmizuki < 2.7) & (diffdnnz < 2.7)
            quality_mask_tune = (diffmizuki < calibration_cut) & (diffdnnz < calibration_cut)
            
            if hsc_catalog == "hscy3_cat_withflags" or hsc_catalog == "hscy3_cat" or hsc_catalog == "hscy3_cat_no_bmode_cut":
                ...
                zmask_data[(~quality_mask_tune & (zvalues <= 0.3)) | 
                       (inside_tomo_range & bad_quality) | 
                       (~quality_mask_tune & (zvalues > 2.0))] = 0  # used only for S19A-standard catalog
            else:
                zmask_data[~quality_mask_tune & (zvalues <= 0.9)] = 0  # used for bins 6 analysis only for non S19A-standard catalog
                # zmask_data[~quality_mask_tune & ((zvalues <= 0.9) | (zvalues > 2.0))] = 0

            assert (
                len(zmask_data[zmask_data > 0]) > 0
            ), "No data left after masking HSC data"

        else:
            zmask_data = np.digitize(cat[self.z_hsc_col], bin_redshift, right=True)
    ...
    def make_cats(self, bin_redshift1=None, bin_redshift2=None):
        ...
        if self.use_desi:
            cat, ran, zmask_cat, zmask_ran = self.set_desi_tracer(
                self.tgt1, bin_redshift1
            )
        if self.double_desi:
            cat2, ran2, zmask_cat2, zmask_ran2 = self.set_desi_tracer(
                self.tgt2, bin_redshift2
            )
        if (
            self.use_hsc and not self.double_desi
        ):  # flags should be incompatible, but just in case
            if self.autocorr:
    ...
    def run_corr(self):
        ...
        """
        raise NotImplementedError("run_corr() not implemented in the derived class. ")

    def save_zeff(self, t1, t2, moc, data_release, hsc_cat):
        """
        Save the effective redshift of the cross correlation.
        """
        if t2 == "HSC":
            outfile = Path(self.output_dir, "zeff", f"{corr_title_corr}zeff_{t1}x{t2}_moc{moc}_{data_release}_{hsc_cat}.npy")
        else:
            outfile = Path(self.output_dir, "zeff", f"zeff_{t1}x{t2}_moc{moc}_{data_release}.npy")
        outfile.parent.mkdir(parents=True, exist_ok=True)
        if outfile.exists() and not overwrite_files:
    ...
    def run(self, bin_index1, bin_index2, moc_index, data_release, hsc_cat):
        """
        Base method to call when running cross corr.
        """
        
        born = config.hsc_keys[0] - 1
        
        desccorr = f"{self.tgt1}x{self.tgt2}"
        if self.tgt2 == "HSC":
            outfile = Path(
                self.output_dir,
                f"{corr_title_corr}{desccorr}_b1x{bin_index1}_b2x{bin_index2 + born}_moc{moc_index}_{data_release}_{hsc_cat}.npy",
            )
        else:
            outfile = Path(
                self.output_dir,
                f"{desccorr}_b1x{bin_index1}_b2x{bin_index2}_moc{moc_index}_{data_release}.npy",
            )
        if outfile.exists() and not overwrite_files:
            self.logger.info(f"File {outfile} already exists, skipping")
            return
        ...
        self.set_current_redshift_masks(bin1=bin_index1, bin2=bin_index2)
    ...
    def make_corr_data(self):
        ...
        if self.use_desi:
            ...
            tz = time.time()
            z1 = (
                self.bin_redshift1[self.bin_index1 - 1]
                + self.bin_redshift1[self.bin_index1]
            ) / 2
            ...
            if self.corr_type == "rp" or self.corr_type == "rppi":
                ...
                dp1.append(self.data1[self.z_desi_col][self.z_bool_d1])
                rp1.append(self.randoms1[self.z_desi_randoms_col])
        else:
            dp1 = [
            ...
            if self.corr_type == "rp" or self.corr_type == "rppi":
                # autocorrelation case : rp1, dp1 are not used
                dp1.append(self.data1[self.z_hsc_col][self.z_bool_d1])
                rp1.append(self.randoms1[self.z_hsc_randoms_col])
        ...
        if not self.autocorr:
            if self.double_desi:
                ...
                if self.corr_type == "rp" or self.corr_type == "rppi":
                    ...
                    dp2.append(self.data2[self.z_desi_col][self.z_bool_d2])
                    rp2.append(self.randoms2[self.z_desi_randoms_col])
            else:
                ...
                if self.corr_type == "rp" or self.corr_type == "rppi":
                    # autocorrelation case : rp2, dp2 are not used
                    dp2.append(self.data2[self.z_hsc_col][self.z_bool_d2])
                    rp2.append(self.randoms2[self.z_hsc_randoms_col])
        else:
            # autocorrelation case : rp2, dp2 are not used
            rp2 = None
            dp2 = None
...
class JackknifeCrossCorrelation(CorrelationMeta):
    def __init__(self, nside: int = 256, nsamples: int = 100, seed: int = 42, **kwargs):
        ...
        if self.corr_type == "rp":
            if self.double_desi:
                rpsamp.append(ct.z2dist(self.randoms2[self.z_desi_randoms_col]))
            else:
                if self.autocorr:
                    rpsamp.append(ct.z2dist(self.randoms1[self.z_desi_randoms_col]))
                else:
                    rpsamp.append(ct.z2dist(self.randoms2[self.z_hsc_randoms_col]))
    ...
    def run_corr(self):
        ...
        tpcf = TwoPointCorrelationFunction(
            ...
            data_samples1=(
                ds1
                if self.estimator_type == "landyszalay"
                else (ds2 if not self.autocorr else ds1)
            ),
            data_samples2=ds2 if self.estimator_type == "landyszalay" else ds1,
            randoms_samples1=(
                rs1
                if self.estimator_type == "landyszalay"
                else (rs2 if not self.autocorr else rs1)
            ),
    ...
    try:
        with fio.FITS(str(f)) as rand:
            ...
            if moc is not None:
                ...
                tc = time.time()
                coords = SkyCoord(
                    data[ra_col] * u.deg, data[dec_col] * u.deg, frame="icrs"
                )
...
def sample_file_on_moc(
    ...
    with fio.FITS(str(file)) as f:
        ...
        if moc is not None:
            print(
                f"Filtering {tbl.get_nrows()} {len(data)} rows in {Path(file).stem} using MOC"
            )
    ...
    assert (
        len(data) > 0
    ), f"No data left after filtering {file} with MOC. Maybe you are using the wrong cap."
...
def _get_data_to_read(
    ...
    if w_cols_to_operate is not None:
        weight_data_sample = {
            col: data[col][:3] for col in w_cols_to_operate if col in data.columns
        }
