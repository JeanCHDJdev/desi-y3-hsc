...
class BuzzardCatalog:
    def __init__(self, buzzard_index):
        ...
        self.ROOT_ALL = Path("/global/cfs/projectdirs/desi/mocks/buzzard/buzzard_v2.0")
        self.ROOT_BUZZARD = Path(
            self.ROOT_ALL, f"buzzard-{self.buzzard_index}/addgalspostprocess"
        )
        ...
        self.files_pix = list(self.TRUTH.glob(f"{self.prefix}*{self.suffix}"))
        self.desi_tgt_files_pix = list(
            self.DESI_TGTS.glob(f"{self.prefix}*{self.suffix}")
        )
        self.pix_nums = sorted(
            [int(pixfile.name.split(".")[1]) for pixfile in self.files_pix]
        )

    def fetch_truth(self, pix):
        return Path(self.TRUTH, f"{self.prefix}{pix}{self.suffix}")
    ...
    def fetch_surveymag(self, pix):
        return Path(
            ...
            (
                ...
                if self.buzzard_index == 4
                else f"surveymags-aux.{pix}{self.suffix}"
            ),
        )

    def fetch_desi_target(self, pix):
        return Path(self.DESI_TGTS, f"{self.prefix}{pix}{self.suffix}")
    ...
    def fetch_desi_randoms(self, target):
        assert target in [
            ...
            "lrg",
            "elg",
        ], "Target must be one of ['BGS', 'LRG', 'ELG']"
        return Path(self.DESI_TGTS_RANDOMS, f"{target}_rand{self.suffix}")
